...
(define (score guess correct)
  (local [(define EXPLODE-CORRECT (explode correct))
          ; letters->LSP : 1String 1String -> LSP
          ; Checks if the guess-letter is Right, Misplaced, or Wrong
          (define (letters->LSP guess-letter correct-letter)
            (make-pair guess-letter
                       (cond [(string=? guess-letter correct-letter) LS-RIGHT] 
                             [(value-in-list? guess-letter EXPLODE-CORRECT string=?) LS-MISPLACED]
                             [else LS-WRONG])))]
    (mymap2 (explode guess) EXPLODE-CORRECT letters->LSP)))
...
(define (partial-guess->full-guess s len)
  (local [(define EXPLODE-S (explode s))]
    (implode (append EXPLODE-S (build-list (- len (length EXPLODE-S)) (λ (_) " "))))))
...
(define (key-wordle ws ke valid-words len)
  (local [(define CURRENT (wordle-current ws))
          (define NEXT (string-append CURRENT (string-upcase ke)))
          (define PAST (wordle-past ws))

          ; remove-tail-letter : String -> String
          ; Strings must not be empty ("")
          ; Deletes the last letter in the string
          (define (remove-tail-letter s)
            (substring CURRENT 0 (sub1 (string-length CURRENT))))
          
          ; backspace-handler : WS [List-of String] -> WS
          ; Updates the worldstate if backspace is pressed.
          (define (backspace-handler ws valid-words)
            (if (empty? (explode CURRENT))
                ws
                (make-wordle PAST (remove-tail-letter CURRENT)))) ; remove last letter
          
          ; return-handler : WS [List-of String] Nat -> WS
          ; Updates the worldstate if return is pressed.
          (define (return-handler ws valid-words len)
            (if (and (= (string-length CURRENT) len) (string-in-list? CURRENT valid-words))
                (make-wordle (append PAST (list CURRENT)) "")
                ws))

          ; alphabet-handler : WS Nat -> WS
          ; Updates the worldstate when a letter key is pressed
          (define (alphabet-handler ws ke len)
            (if (and (< (string-length CURRENT) len) (string-alphabetic? ke))
                (make-wordle PAST NEXT)
                ws))]
    (cond [(key=? ke "\b") (backspace-handler ws valid-words)]
          [(key=? ke "\r") (return-handler ws valid-words len)]
          [else (alphabet-handler ws ke len)])))
...
(define (end-wordle? ws correct num-guesses)
  (or (= (length (wordle-past ws)) num-guesses)
...
(check-expect (draw-end-wordle WS-WORD2 "DAB" 2)
              (overlay/align
               ...
               (overlay/align "middle" "top"
                              (text "2 guesses" 20 "black")
                              (overlay (text "good job!" 30 "white")
                                       (square 200
                                               "solid"
                                               "green")))))

(check-expect (draw-end-wordle WS-WORD3 "DAB" 3)
              (overlay/align
               "middle" "bottom"
               (text "DAB" 20 "black")
               (overlay/align "middle" "top"
                              (text "3 guesses" 20 "black")
                              (overlay (text "good job!" 30 "white")
                                       (square 200
                                               "solid"
                                               "green")))))

(check-expect (draw-end-wordle WS-WORD3 "MAC" 3)
              (overlay/align
               "middle" "bottom"
               (text "was MAC" 20 "black")
               (overlay/align "middle" "top"
                              (text "the correct word" 20 "black")
                              (overlay (text "u lost :(" 30 "white")
                                       (square 200
                                               "solid"
                                               "red")))))

(define (draw-end-wordle ws correct num-guesses)
  (local [(define WS-WINNING? (string-in-list? correct (wordle-past ws)))
          (define WIN-SCREEN (overlay/align
                              "middle" "bottom"
                              (text correct 20 "black")
                              (overlay/align "middle" "top"
                                             (text (string-append (number->string num-guesses)
                                                                  " guesses")
                                                   20 "black")
                                             (overlay (text "good job!" 30 "white")
                                                      (square 200 "solid" "green")))))
          (define LOSE-SCREEN (overlay/align
                               "middle" "bottom"
                               (text (string-append "was " correct) 20 "black")
                               (overlay/align "middle" "top"
                                              (text "the correct word" 20 "black")
                                              (overlay (text "u lost :(" 30 "white")
                                                       (square 200 "solid" "red")))))]
    (if WS-WINNING? WIN-SCREEN LOSE-SCREEN)))
...
; prior-guesses :  WS String -> [List-of Image]
; Creates a list of rows representing the prior guesses

(check-expect (prior-guesses WS-START "BAD") '())
(check-expect (prior-guesses WS-TYPE1 "BAD") '())
(check-expect (prior-guesses WS-TYPE2 "BAD") '())
(check-expect (prior-guesses WS-TYPE3 "BAD") '())
(check-expect (prior-guesses WS-WORD1 "BAD")
              (list
               (row->image (map (λ (lsp) (make-pair lsp lsp->image)) (score "CAT" "BAD")))))
(check-expect (prior-guesses WS-WORD2 "BAD")
              (list
               (row->image (map (λ (lsp) (make-pair lsp lsp->image)) (score "CAT" "BAD")))
               (row->image (map (λ (lsp) (make-pair lsp lsp->image)) (score "DAB" "BAD")))))

(define (prior-guesses ws correct)
  (local [; visually-score-guess : String -> Image
          ; Visualizes the letters in the given string after scoring based on the correct answer
          (define (visually-score-guess pastguess)
            (row->image (map (λ (lsp) (make-pair lsp lsp->image)) (score pastguess correct))))]
    (map visually-score-guess (wordle-past ws))))

; current-guess : WS String -> Image
; Visualizes current partial guess

(check-expect (current-guess WS-START "CAT") (partial-guess->image "" 3))
(check-expect (current-guess WS-TYPE1 "CAT") (partial-guess->image "C" 3))
(check-expect (current-guess WS-TYPE2 "CAT") (partial-guess->image "CA" 3))
(check-expect (current-guess WS-WORD1 "CAT") (partial-guess->image "" 3))
(check-expect (current-guess WS-WORD2 "CAT") (partial-guess->image "" 3))

(define (current-guess ws correct)
  (partial-guess->image (wordle-current ws) (string-length correct)))
               
; future-guesses : WS String Nat -> [List-of Image]
; Creates a list of rows of blank spaces for each remaining guess

(check-expect (future-guesses WS-START "CAT" 5) (build-list
                                                 (- 5 1 (length (wordle-past WS-START)))
                                                 (λ (_) (partial-guess->image "" 3))))
(check-expect (future-guesses WS-TYPE1 "CAT" 5) (build-list
                                                 (- 5 1)
                                                 (λ (_) (partial-guess->image "" 3))))
(check-expect (future-guesses WS-TYPE2 "CAT" 5) (build-list
                                                 (- 5 1)
                                                 (λ (_) (partial-guess->image "" 3))))
(check-expect (future-guesses WS-WORD1 "CAT" 5) (build-list
                                                 (- 5 1 (length (wordle-past WS-WORD1)))
                                                 (λ (_) (partial-guess->image "" 3))))
(check-expect (future-guesses WS-WORD2 "CAT" 5) (build-list
                                                 (- 5 1 (length (wordle-past WS-WORD2)))
                                                 (λ (_) (partial-guess->image "" 3))))


(define (future-guesses ws correct max-guesses)
  (build-list (- max-guesses 1 (length (wordle-past ws)))
              (λ (_) (partial-guess->image "" (string-length correct)))))
...
(check-expect (draw-wordle WS-START "CAT" 5)
              (overlay
               (stack/v (append (prior-guesses WS-START "CAT")
                                (list (current-guess WS-START "CAT"))
                                (future-guesses WS-START "CAT" 5))) (square 200 "solid" "white")))

(check-expect (draw-wordle WS-TYPE1 "CAT" 5)
              (overlay
               (stack/v (append (prior-guesses WS-TYPE1 "CAT")
                                (list (current-guess WS-TYPE1 "CAT"))
                                (future-guesses WS-TYPE1 "CAT" 5))) (square 200 "solid" "white")))

(check-expect (draw-wordle WS-WORD1 "CAT" 5)
              (overlay
               (stack/v (append (prior-guesses WS-WORD1 "CAT")
                                (list (current-guess WS-WORD1 "CAT"))
                                (future-guesses WS-WORD1 "CAT" 5))) (square 200 "solid" "white")))

(check-expect (draw-wordle WS-WORD2 "CAT" 5)
              (overlay
               (stack/v (append (prior-guesses WS-WORD2 "CAT")
                                (list (current-guess WS-WORD2 "CAT"))
                                (future-guesses WS-WORD2 "CAT" 5))) (square 200 "solid" "white")))

(define (draw-wordle ws correct max-guesses)
  (overlay (stack/v
            (append (prior-guesses ws correct)
                    (list (current-guess ws correct))
                    (future-guesses ws correct max-guesses))) (square 200 "solid" "white")))
